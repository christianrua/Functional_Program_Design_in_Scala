...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.b1 == goal && b.b2 == goal //b.isStanding && b.b1 == goal
  ...
  /*
  b.legalNeighbors => (block, move)
    b.legalNeighbors.map{
    case (block, move) => (block, move::history) #:: neighborsWithHistory(b,history)
  }
*/
  ...
  /*
    for(
      ...
      finalList <- LazyList((block, move::history), LazyList.empty)
      ) yield finalList
*/
  ...
  {
    neighbors filterNot { case (block, ms) => explored contains block }
  }
  /*
    for {
 ...
 */
  ...
  def from(initial: LazyList[(Block, List[Move])],explored: Set[Block]): LazyList[(Block, List[Move])] =
    initial match {
      ...
      case (block, ms) #:: rest => {
        val more = newNeighborsOnly(neighborsWithHistory(block, ms), explored + block)
  ...
  /*
    initial match {
      ...
      case (block, move) #:: rest => {
        val more = newNeighborsOnly(neighborsWithHistory(block, move), explored + block)
        more ++ from(rest ++ more, explored + block)
      }
    }
*/
...
*/
  /*
  {
    if (initial.isEmpty) LazyList.empty
      ...
      initial append from(more, explored ++ more.map{case (block, _) => block})
    }
  }

  {
        if (paths.isEmpty) LazyList.empty
            ...
            val more = for {
               ...
               if !(explored contains next.endSate)
            } yield next
        paths #:: from(more,explored++more.map(_.endSate))
    }
}
   */
